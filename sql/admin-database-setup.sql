...
CREATE POLICY "Admin users can view their own data" ON admin_users
    FOR SELECT USING (true); -- Allow all authenticated admins to view

CREATE POLICY "Admin users can update their own data" ON admin_users
    FOR UPDATE USING (true); -- Allow all authenticated admins to update

CREATE POLICY "Admin users can insert new admins" ON admin_users
    FOR INSERT WITH CHECK (true); -- Allow authenticated admins to create new admins
...
-- 11. Insert default admin user (password: admin123 - CHANGE THIS!)
